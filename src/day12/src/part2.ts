...
function main() {
    ...
    let bestAnswer: AStarNode | null = null;
    for (const startNode of startNodes) {
        const result = AStarNode.aStarTimeBaby(startNode, endNode!);
        if (result && (bestAnswer == null || bestAnswer.computedCost > result?.computedCost)) {
            bestAnswer = result;
            // results.push(result);
            console.log(result.computedCost);
        }
    }
    // printAnswerIHope(bestAnswer!);
...
function printAnswerIHope(node: AStarNode) {
    const buffer: string[][] = [];
    let currentNode: AStarNode | undefined = node;
    while (currentNode != null) {
        if (buffer[currentNode.innerNode.rowIdx] == null) {
            buffer[currentNode.innerNode.rowIdx] = [];
        }
        const parent: AStarNode | undefined = currentNode.parent;
        if (parent) {
            if (parent.innerNode.rowIdx > currentNode.innerNode.rowIdx) {
                buffer[currentNode.innerNode.rowIdx][currentNode.innerNode.colIdx] = "<";
            }
            if (parent.innerNode.rowIdx < currentNode.innerNode.rowIdx) {
                buffer[currentNode.innerNode.rowIdx][currentNode.innerNode.colIdx] = ">";
            }
            if (parent.innerNode.colIdx > currentNode.innerNode.colIdx) {
                buffer[currentNode.innerNode.rowIdx][currentNode.innerNode.colIdx] = "^";
            }
            if (parent.innerNode.colIdx < currentNode.innerNode.colIdx) {
                buffer[currentNode.innerNode.rowIdx][currentNode.innerNode.colIdx] = "V";
            }
        }
        currentNode = parent;
    }
    const printableMap = buffer.map(item => item.join("")).join("\n");
    Deno.writeTextFileSync("test.txt", printableMap);
}

function printMapHopefully(map: MapNode[][]) {
  const buffer: string[][] = [];
  map.forEach((row, rowIdx) => {
    row.forEach((node, colIdx) => {
      const bufferRow = rowIdx * 2;
      const bufferCol = colIdx * 2;
      if(buffer[bufferRow] == null) {
        buffer[bufferRow] = [];
      }
    //   buffer[bufferRow][bufferCol] = (node.height - 97).toString().padStart(2, "0");
    buffer[bufferRow][bufferCol] = ".";

      for(const connection of node.connections) {
        if(buffer[bufferRow - 1] == null) {
          buffer[bufferRow - 1] = [];
        }
        if(buffer[bufferRow + 1] == null) {
          buffer[bufferRow + 1] = [];
        }
        if(connection.rowIdx < node.rowIdx) {
          if(buffer[bufferRow - 1][bufferCol] == null) {
            buffer[bufferRow - 1][bufferCol] = '^';
          } else {
            buffer[bufferRow - 1][bufferCol] = '|';
          }
        }
        if(connection.rowIdx > node.rowIdx) {
          if(buffer[bufferRow + 1][bufferCol] == null) {
            buffer[bufferRow + 1][bufferCol] = 'V';
          } else {
            buffer[bufferRow + 1][bufferCol] = '|';
          }
        }
        if(connection.colIdx < node.colIdx) {
          if(buffer[bufferRow][bufferCol - 1] == null) {
            buffer[bufferRow][bufferCol - 1] = '<';
          } else {
            buffer[bufferRow][bufferCol - 1] = '=';
          }
        }
