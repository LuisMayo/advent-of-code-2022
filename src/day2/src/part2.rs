...
impl Match {
    pub fn new (str: &str) -> Self {
        ...
        let my_needs = char_iterator.skip(1).next().unwrap();
        let my_choose: char;
        match my_needs {
            NEED_LOSE => match their_choose {
                THEIR_ROCK => my_choose = MY_SCISSORS,
                THEIR_PAPER => my_choose = MY_ROCK,
                THEIR_SCISSORS => my_choose = MY_PAPER,
                _ => panic!("Ilegal move by them")
            },
            NEED_DRAW => match their_choose {
                THEIR_ROCK => my_choose = MY_ROCK,
                THEIR_PAPER => my_choose = MY_PAPER,
                THEIR_SCISSORS => my_choose = MY_SCISSORS,
                _ => panic!("Ilegal move by them")
            },
            NEED_WIN => match their_choose {
                THEIR_ROCK => my_choose = MY_PAPER,
                THEIR_PAPER => my_choose = MY_SCISSORS,
                THEIR_SCISSORS => my_choose = MY_ROCK,
                _ => panic!("Ilegal move by them")
            },
            _ => panic!("Ilegal match result")
        }
